...
export class NotificacionService {
	...
	listarPorUsuario(usuarioId: string): Promise<Notificacion[]> {
		...
		return new Promise(function(resolver, rechazar) {
			servicioActual.http.get<RespuestaApi<Notificacion[]>>(`${servicioActual.urlBase}/usuario/${usuarioId}`)
				.subscribe({
	...
	listarNoLeidas(usuarioId: string): Promise<Notificacion[]> {
		const servicioActual = this;
		return new Promise(function(resolver, rechazar) {
			servicioActual.http.get<RespuestaApi<Notificacion[]>>(`${servicioActual.urlBase}/usuario/${usuarioId}/no-leidas`)
				.subscribe({
					next: function(respuesta: RespuestaApi<Notificacion[]>) {
	...
	obtenerConteoNoLeidas(usuarioId: string): Promise<number> {
		const servicioActual = this;
		return new Promise(function(resolver, rechazar) {
			servicioActual.http.get<RespuestaApi<number>>(`${servicioActual.urlBase}/usuario/${usuarioId}/contar-no-leidas`)
				.subscribe({
	...
	marcarComoLeida(notificacionId: string): Promise<void> {
		const servicioActual = this;
		return new Promise(function(resolver, rechazar) {
			servicioActual.http.put<RespuestaApi<void>>(`${servicioActual.urlBase}/${notificacionId}/marcar-leida`, {})
				.subscribe({
					next: function() {
	...
	marcarTodasComoLeidas(usuarioId: string): Promise<void> {
		const servicioActual = this;
		return new Promise(function(resolver, rechazar) {
			servicioActual.http.put<RespuestaApi<void>>(`${servicioActual.urlBase}/usuario/${usuarioId}/marcar-todas-leidas`, {})
				.subscribe({
					next: function() {
	...
	eliminarNotificacion(notificacionId: string): Promise<void> {
		const servicioActual = this;
		return new Promise(function(resolver, rechazar) {
			servicioActual.http.delete<RespuestaApi<void>>(`${servicioActual.urlBase}/${notificacionId}`)
				.subscribe({
					next: function() {
						const notificacionesActuales = servicioActual.notificacionesDelUsuario();
	...
	private actualizarNotificacionLocal(notificacionId: string, ahora_Leida: boolean): void {
		const notificacionesActuales = this.notificacionesDelUsuario();
		const notificacionActualizada = notificacionesActuales.map(notif => 
			notif.id === notificacionId ? { ...notif, leida: ahora_Leida } : notif
		);
