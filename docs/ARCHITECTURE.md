...
- **SSH a VPS remotos**: punto único `SshCommandService.ejecutarComando(servidor, comando)` con autenticación por contraseña o clave privada (ambas cifradas con AES-GCM en MongoDB).
- **SFTP**: `SftpUploadService` para subir ZIPs antes del build remoto.
- **Cifrado**: `CifradoUtil` con **AES-256/GCM/NoPadding** (IV aleatorio de 12 bytes prepended al ciphertext + tag de autenticación de 128 bits) usando `AUTODEPLOY_CIFRADO_CLAVE` derivada con SHA-256.
...
### Cifrado AES-GCM en lugar de almacenar credenciales en plano

Cualquier filtración de la BBDD comprometería todos los VPS de todos los usuarios si se guardasen las contraseñas/claves SSH en plano. Se cifra con **AES-256/GCM/NoPadding** (IV aleatorio de 12 bytes + tag de autenticación de 128 bits) usando `AUTODEPLOY_CIFRADO_CLAVE` (env var, no en BBDD), y se descifra solo en memoria justo antes de abrir la conexión SSH.

GCM se elige sobre el clásico CBC + HMAC porque ofrece autenticación nativa: si el ciphertext fuera modificado en BBDD (tampering), el descifrado falla con `AEADBadTagException`. Además se evita el patrón ECB del primer borrador (mismo plaintext → mismo ciphertext, vulnerable a análisis de frecuencia). Los registros antiguos cifrados con AES/ECB siguen siendo legibles gracias a un descifrador legacy de fallback.
