...
describe("Cuenta", function () {
  ...
  it("cambiarPlan a 'free' desde un plan de pago cancela la suscripcion", async function () {
    ...
    peticion.flush({
      success: true,
      message: "OK",
      data: { id: "507f1f77bcf86cd799439011", nombre: "Pepe", email: "pepe@test.com", token: null, plan: "free", fechaFinSuscripcion: "2026-12-31", idioma: "es" }
    });
  ...
  it("guardarCambios llama al servicio y muestra mensaje guardado", fakeAsync(function () {
    ...
    peticion.flush({
      success: true,
      message: "OK",
      data: { id: "507f1f77bcf86cd799439011", nombre: "PepeNuevo", email: "nuevo@test.com", token: null, plan: "pro", fechaFinSuscripcion: null, idioma: "es" }
    });
    tick();
  ...
  it("cargarClavesYNotificaciones no pide nada si no hay usuarioId al crear", async function () {
    ...
    httpMock.expectNone(function (req) {
      return req.url.includes("/claves-ssh");
    });
